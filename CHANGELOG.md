...
- promote-release: candidate promotion governed by `sign-tags` — `manual` defers to the release manager (implied by an active signature-requiring tag ruleset, and by unreadable rulesets), `off` lets the pipeline tag with the marker's message, publish the draft, and advance the moving major in one job. The major advances only for the highest stable in its line — decided before anything goes live, so a backport publishes without touching it.
