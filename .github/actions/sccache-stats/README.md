...
Record what sccache delivered as `cache.sccache.*` facts in the step summary and for the `timing-report` Cache section, plus the raw `sccache --show-stats` output in a log group.
...
- The facts render as a small table in the step's own summary, so they show without `timing-report`.
