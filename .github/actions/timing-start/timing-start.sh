...
#
# Not -e: instrumentation must never fail the job it measures.
set -uo pipefail
...
dir="$(timing_dir)"
# Re-running timing-start in one job would otherwise let two samplers
# interleave into one file and put the job's start in the wrong place. Stop
# the earlier sampler, start clean and say so.
if grep -q "^started	" "$dir/notes.tsv" 2>/dev/null; then
  echo "::warning title=timing-start ran twice::discarding the earlier samples and notes; only the last timing-start in a job is measured"
...
rm -f "$dir/boundaries.tsv" "$dir/stages.tsv" "$dir/stages.tsv.sorted" "$dir/sampler.pid"

# The job's start, for the totals the report falls back to without the API.
timing_note "started" "$(timing_now_ms)"
