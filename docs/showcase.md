...
The pipeline gates with `check-release-readiness`, builds, then `draft-release` creates the draft pre-release and pushes an unsigned candidate marker, and `release-guidance` writes the next steps into the step summary.
A human signs and pushes the final tag; `require-signed-tag` gates the tag run, `publish-draft-release` seals the tag against the newest marker, publishes the draft and advances the moving major.
...
## Publish: crates.io behind a signature

`require-signed-release` answers whether a verified human signature covers the release commit; unsigned is an answer, not a failure.
`cargo-publish` uploads only when that answer is true and only for versions matching its `tag-pattern`, with the Trusted Publishing token from `rust-lang/crates-io-auth-action` passed as `registry-token`; otherwise it rehearses with `cargo publish --dry-run`.
...
- Signed tags: import `.github/rulesets/tags-maintainer-only.json` so only maintainers create `v*` tags, and require a verified signature on the tag the pipeline gates on.
